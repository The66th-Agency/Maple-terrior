...
**Still pending (not done):** (a) the Era-2 per-article blog redirect (`/blog/post?handle=X` still 301s to the `/blog/` hub, not the article) still needs the dashboard Single Redirect Rule from the 2026-05-30 note below, still not added; (b) `Organization.sameAs` in `index.html` is empty `[]` and the site has zero social links anywhere (grep found only a YouTube video embed). Needs Liam's real social profile URLs before populating; do NOT fabricate. Full audit artifacts saved to `~/Downloads/maple-terroir-seo-audit-2026-06-01/`.
