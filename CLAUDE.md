...
- GA4 IS live on MT (gtag `G-2HETD7E9WS`, numeric property `534540699`; verified pulls 2026-07-07 via `ga4_report.py --property 534540699`). The claude-seo config default is still the66th's property, so always pass MT's id explicitly. PostHog also runs in parallel.
...
- **PENDING (2026-07-03): Liam needs to get backlinks to `https://mapleterroir.com/blog/best-canadian-maple-syrup-brands-2026`.** It is MT's fastest-growing page (0 to 291 clicks in 6 weeks, pos 6-7 on "best maple syrup in canada" / "best canadian maple syrup"); links are the remaining lever to reach top 3, all on-page work is done. Remind Liam of this on any MT SEO session until links are acquired, then delete this item.

## Lab Notes
[date] [what happened] [what to do differently]

2026-07-07 (later, same day): **_redirects ORDERING is load-bearing: wrangler counts every rule after the first wildcard as "dynamic" (cap 100) and deploys hard-fail past it.** The audit batch grew _redirects from 78 to 121 rules; with /cart/* wildcards at the top of the file, the parser's one-way canCreateStaticRule flag made ALL 121 count as dynamic and every deploy (Workers Builds AND manual wrangler) failed with "Maximum number of dynamic _redirects rules limit of 100 exceeded". I initially misdiagnosed this as Cloudflare's Jul 4 build-system migration; the real evidence came from running `npx wrangler deploy` locally, which prints the actual validation error that Workers Builds hides behind a dashboard login. **Rules:** (1) _redirects now has two labeled sections: exact rules first (static, limit 2,000), wildcard rules last (dynamic, limit 100). Append new exact rules to section 1, never above a wildcard. (2) When a Workers Build fails and the log is unreachable, `npx wrangler deploy --dry-run` then a real deploy locally reproduces the server-side validation error. (3) wrangler.jsonc now lives in the repo (cherry-picked from Cloudflare's autoconfig branch); manual `npx wrangler deploy` works as a bypass when approved.

2026-07-07: **Full 12-agent SEO audit (health 69/100) + fix batch shipped in one pass.** Audit checkpoint lives in the memory doc `project_maple_terroir_seo_audit.md`; raw evidence in that session's scratchpad. What shipped: (1) all internal `.html` nav/footer/body links rewritten to clean URLs (1,710 rewrites, 73 files) + explicit 301s in `_redirects` for every legacy `.html` form; (2) NEW `scripts/build-grids.mjs` bakes static crawlable product cards into all 10 `collections/*.html` grids and a full-catalog grid into `products.html` (106 product links now in raw HTML; page JS still hydrates the live view on load). **Re-run it alongside build-products.mjs on any catalog change.** (3) build-products.mjs now also emits BreadcrumbList JSON-LD and bakes the visible price into `#product-price` (raw HTML used to show an empty span), and its `absolutize()` no longer mangles JS string concatenations (`src="/https://...` bug) or empty `src=""`; (4) footer "Shop" column now links `/canadian-maple-syrup` + `/organic-maple-syrup` sitewide (53 files) because both money pages were internally orphaned; (5) blog fixes: internal links no longer `target="_blank"`, legacy `/blogs/blogs-maple-terroir/` hrefs rewritten, FAQPage schema added to the tap-tree post; (6) Canada city pages' BreadcrumbList had position 2 and 3 both pointing at the page itself; fixed to the `/locations-we-serve/` hub (USA pages were already correct); (7) fonts self-hosted at `assets/fonts/` (Google Fonts links removed sitewide, ~750ms render-block gone; CSP already allowed 'self'); (8) 3 oversized PNGs (usda-cert, pantry-bento, maple-tea-bento: 700-850KB each) converted to sized WebP (19-209KB); (9) homepage title rebalanced to brand-led "Maple Terroir | Single-Origin Quebec Maple Syrup" so `/canadian-maple-syrup` can own the head term; (10) sitemap: `/privacy` added, lastmod bumped on the 63 changed entries. GOTCHA that triggered all this: 7 of 40 PDPs were stale-wrong in Google's index (4 as 404, 2 with old `?handle=` canonicals folding into /products) while all 7 served live 200s; Google had not recrawled since mid-May because PDPs had near-zero internal link equity (JS-only grids). After any indexation fix here, request recrawl and give it weeks. NOT done in this batch (pending): merge/backfill the three 2-SKU collections (tea-coffee, nuts-snacks, maple-syrup-gift-set; recommendation doc for Shawn includes the required 301s), tourist-page duplicate "What to Bring Home" block, PDP reviews/social proof, named blog authors, 404.html not being served (needs a Worker `not_found_handling` dashboard/wrangler setting, not a repo file).

2026-06-29: **Retailer logos on DTC pages are SOCIAL PROOF, not an order-elsewhere CTA.** Liam flagged the reworked location-page "Delivered to Your Door" sections: showing Amazon/Costco/Walmart logos with copy like "you can also find Maple Terroir on Amazon, Costco, and Walmart" pushes buyers OFF the site, when the whole point is to sell direct on mapleterroir.com. **Rule:** third-party retailer logos exist to build trust (the brand is carried by major retailers), never to route the sale away. Frame them under a small understated "Trusted and Stocked By" label, keep them UNLINKED, and make the primary prominent action an on-site buy (a "Shop the Full Range" button to /collections/premium-maple-syrup). Order-direct is always the hero; the logo strip is the trust signal beneath it. Applies to every MT page that shows retailer logos.

2026-06-29: **Logo-row monochrome normalization (Shawn feedback): single-colour wordmarks filter to black, multi-colour badges must be REPLACED.** The "Where to Find Us" strip in [index.html](index.html) holds 9 retailer logos meant to read as a uniform black wordmark row. Two were off-colour. **Canex** is a one-colour red wordmark, so a CSS rule `img[alt="CANEX"] { filter: brightness(0); }` blackens it cleanly. **Nesters** is a full-colour badge (mascot, wood texture, outlined text) on a baked-in WHITE background, so `brightness(0)` turns it into an unreadable black blob AND the white box breaks the cream card. Fix: generated a transparent black "NESTERS MARKET" wordmark with PIL (Arial Bold, letterspaced, stacked two lines to match Urban Fare's aspect, ~2.35:1 so object-contain renders it at full box height) and overwrote `assets/images/logos/nesters.png` (no HTML change; all 4 marquee copies pick it up). **Rules:** (1) To monochrome a logo row, CSS-filter single-colour wordmarks but REPLACE multi-colour/badge logos with a transparent black wordmark asset, never filter them. (2) A very wide wordmark (~10:1) renders small under `object-contain` in the ~176px box; stack it to ~2.5:1 to match the row's visual weight. (3) Source logo PNGs with baked white backgrounds look wrong on the cream cards regardless of colour; always use transparent. (4) Finish a logo fix in ONE pass: I shipped Canex and left Nesters "queued," so Shawn/Liam saw a half-fixed row and had to flag it.

2026-06-28: **Verified MT's real retail footprint (adversarial deep research) and found the Montreal location page makes unverifiable stockist claims.** Two background agents (research + adversarial verifier) checked every claimed Montreal stockist against the RETAILER's own site. Result: **0 of 9 confirmed.** Premiere Moisson sells its own house brand (no Maple Terroir), Montreal Duty Free and Souvenir du Quebec stock 6 other brands but not MT, and Délices Érable appears CLOSED since May 2026. Every Montreal stockist claim traces back ONLY to MT's own self-published `locations-we-serve/canada/buy-maple-syrup-montreal` page, which is a marketing claim, not a primary source. MT is actually **B.C.-based** (BC Agrifood Export Catalogue lists "Maple Terroir Products" as B.C., sourcing from a Quebec farm); its real retail footprint is Western/national chains: **Costco, Save-On-Foods, Urban Fare, Shoppers Drug Mart, HomeSense, CANEX** (these surface OUTSIDE MT's own site, so they pass; the `best-canadian-maple-syrup-brands-2026` blog already cites these correctly). **Rules going forward:** (1) An "available at retailer X" claim on any location page must trace to the RETAILER's own site, never MT's self-published page; the Montreal page's Premiere Moisson/Provigo/Metro/Délices Érable line fails the fact-check gate and should be softened to DTC + public markets. (2) Geo pSEO Layer 1 must LEAD with Vancouver / Western Canada (real retail presence), NOT Montreal / Eastern cities (DTC-only there = doorway-page risk). (3) The search-result snippet claiming "MT available at Premiere Moisson, Provigo, Metro" was MT's own page being quoted back; do not treat a brand's self-published location page as third-party proof.

2026-06-07 (changelog): **Homepage SEO + perf fixes, all shipped to `main` and verified live on mapleterroir.com.**
- **Globe restored** (`01a7ac6`): self-hosted three.js r128 + topojson + world-atlas under `assets/vendor/`; it had been blank ~6 weeks behind the CSP. Full detail in the 2026-06-06 note below.
- **Product schema deduped** (`2717e5b`): removed the runtime-injected second Product JSON-LD from `product.html`, kept the build-baked one, regenerated all 40 `products/*.html`.
- **Invalid GTIN gated** (`2717e5b`): `scripts/build-products.mjs` now emits a GTIN only for a real numeric barcode (it was baking `"gtin":"custom product"`).
- **robots.txt** (`2717e5b`): named the major AI crawlers with explicit Allow (GPTBot, OAI-SearchBot, ChatGPT-User, ClaudeBot, Claude-SearchBot, PerplexityBot, Google-Extended, Applebot-Extended).
- **Tapping blog article overhauled** (`20e0465`): real meta + Article-schema description (was the generic auto-fallback), a Quick Answers box, a sap-to-syrup ratio table by species, and removed an empty `<h1 data-pm-slice>` artifact. Edited the served static `blog/<slug>.html` directly; see the regenerate caveat in the note below.
- **Hero video loads fast** (`b7a4b9b`): `preload` changed from `metadata` to `auto`, plus a desktop-only `<link rel="preload" as="video" media="(min-width:769px)">`. It had been showing the poster for ~5s before the JS-assigned src buffered. Still off on phones.
- **Bento background videos lazy-loaded** (`fd019a7`): the two autoplay bento videos (~4.8MB) were downloading on page load while below the fold. Removed autoplay, set `preload="none"`, added a `.bento-video` IntersectionObserver that plays each on scroll-in (desktop only, phones get posters), and gave the chocolates card a charcoal background since it had no poster.
- **Still open (not blockers):** the tapping article's body still links to old `/blogs/blogs-maple-terroir/` URLs (they 301, tidy later); the shared poster `public(18).png` (524KB, messy name at repo root) could move into `assets/`; this machine has no ffmpeg, so video posters were not set to each video's own first frame.

2026-06-07: **SEO fix session: I tried to offload my own work to Liam and re-asked for a push he had already approved. Both made him escalate.** Two mistakes never to repeat: (1) I told Liam to paste blog content into Shopify, and (2) after he approved the plan (commit + push + deploy) I asked again before pushing (the auto-approve classifier had also blocked the push, reading the earlier "so I can sign off" as a still-live gate). **How this site is actually edited:** the live site serves the repo's static files (see Deployment & Environments). Blog and content edits are made by editing those static files DIRECTLY from Claude Code, then commit + push. The Shopify Storefront token in the repo is READ-only (hydration only); we do NOT edit Shopify content in normal ops. So never send Liam into Shopify for something a repo edit does, and never re-ask for a push/commit/deploy he has already approved, just do it and hand back the live URL. **Blog regenerate caveat:** `_scripts/generate-static-blogs.mjs` is a one-way manual import from Shopify. If you hand-edit a served `blog/<slug>.html` (as I did for the "can you tap any maple tree" article: real meta + Article-schema description, a Quick Answers box, a sap-to-syrup ratio table, and removed an empty `<h1 data-pm-slice>` ProseMirror artifact), do NOT re-run that generator for the handle without first putting the same content into the Shopify article, or it silently reverts. **Same session, product fixes (in the repo, the right way):** removed the runtime-injected duplicate Product JSON-LD from `product.html` (kept the build-baked one), and gated `build-products.mjs` so it only emits a GTIN for a real numeric barcode (it was baking `"gtin":"custom product"`). Regenerate products with `node scripts/build-products.mjs`.

2026-06-06: **Homepage globe was blank for ~6 weeks; a CSP had quietly blocked its CDNs.** The Global Reach 3D globe ([index.html](index.html), Three.js) loads three.js from cdnjs and topojson + the world-atlas `land-110m.json` from jsdelivr. The 2026-04-22 "Security hardening" commit (`82a0047`) added a CSP whose `script-src` (PostHog/GTM only) and `connect-src` (Shopify/analytics only) whitelisted none of those origins, so the browser blocked three.js, `window.THREE` never defined, `waitForThree()` polled forever, and the `#globe-canvas` stayed empty. Nothing errored out loud; the page still loaded, so nobody noticed for six weeks. The globe shipped 2026-04-06 (`b1aa850`) and worked until the CSP landed 16 days later. **Fix (shipped to main, verified live):** self-hosted all three under `assets/vendor/` (version-pinned: three r128, topojson v3, world-atlas v2) and repointed index.html, so everything loads from `'self'`, which the existing CSP already allows. No CSP widening. Added an immutable cache rule for `/assets/vendor/*` in `_headers`. **Rules going forward:** (1) Any CSP add or tighten must be diffed against EVERY external script/fetch/font/img/media the pages actually use, not just analytics; a missed origin kills that feature silently. (2) Prefer self-hosting a third-party lib over whitelisting its CDN (same reasoning that retired the Tailwind runtime CDN: an external CDN is a silent single point of failure). (3) A blocked third-party widget throws no visible error, so after any CSP change, confirm each dependent feature still renders on a real browser. (4) The headless preview tool has NO WebGL (`getContext('webgl')` returns null), so the globe cannot be screenshot-verified locally; verify the libs load (`THREE.REVISION`) + assets return 200, then eyeball it on a real browser.
...
2026-08-13: **The May quantity-button fix only ever landed on the product template; the other 33 pages kept the old laggy handler and kept generating rage clicks.** PostHog logged 17 rage clicks/30d on the cart drawer's quantity +/- controls. Live repro on /products: three fast taps left the counter frozen ~2.5s, then it settled at +1 (two taps swallowed, zero feedback). Cause: the cart drawer is pasted inline into every page, and the 2026-05 optimistic-update fix went only into product.html (and its 40 generated PDPs). Fix: ported the optimistic debounced handler (instant DOM update, 300ms coalesce per line via window.__mtQtyTimers, reconcile-after) to all 33 remaining pages, which exist in TWO dialects (long names: lines/renderDrawer/getLines, minified: ls/draw/lines()). Verified per-tap response locally on both dialects before push. **Rules:** (1) The cart drawer is a SYSTEM duplicated across ~73 files in two naming dialects; any drawer change must sweep every file carrying `data-action="inc"` in ONE commit (same principle as the nav rule and the 2026-03-30 bento rule). (2) Standing refactor candidate: move the drawer into assets/shared.js so a fix lands once; until then rule 1 is the guard.
