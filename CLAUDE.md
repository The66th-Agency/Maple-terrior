...
- (b) **PRODUCT twin NOT added yet.** The analogous rule for `/product[.html]?handle=X` -> `/products/<handle>` is not built. Caveat: discontinued handles (e.g. `maple-tea`) have no static page, so a per-product rule would 301 them into a 404. Redirect dead product handles in `_redirects` first, THEN add the product rule. Lower priority than the blog rule was (product `?handle=` URLs were not in the GSC top-traffic pages).
- (c) `Organization.sameAs` in `index.html` is still empty `[]`; site has zero social links (grep found only a YouTube embed). Needs Liam's real social URLs before populating; do NOT fabricate.
